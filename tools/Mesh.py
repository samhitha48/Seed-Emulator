...
def get_pod_spec(name, image, ip, bird_conf=None):
    container = {
        ...
        "command": ["sh", "-c", f"ip addr add {ip}/24 dev eth0 && mkdir -p /run/bird && bird -c /etc/bird/bird.conf -s /run/bird/bird.ctl && sleep infinity"],
        "volumeMounts": [] if not bird_conf else [{
            "mountPath": "/etc/bird",
            "name": f"{name}-bird-config"
        }]
    }
    ...
    pod = {
        ...
        "spec": {
            ...
            "volumes": [] if not bird_conf else [{
                "name": f"{name}-bird-config",
                "configMap": { "name": f"{name}-bird-config" }
            }]
        }
    }
    return pod
...
def get_configmap(name, bird_conf_path):
    ...
    return {
        "apiVersion": "v1",
        "kind": "ConfigMap",
        "metadata": { "name": f"{name}-bird-config" },
...
def main(output_path):
    ...
    for pod_dir in output.iterdir():
        ...
        # Fallback IP generator (dummy if ip.txt not found)
        ip = "10.105.0." + str(hash(name) % 200 + 50)
        if ip_path.exists():
            ip = ip_path.read_text().strip()

        pods.append(get_pod_spec(name, image, ip, bird_path if bird_path.exists() else None))

        if bird_path.exists():
            configmaps.append(get_configmap(name, bird_path))

        #connect all pods to all
        for other in pods:
            if other["metadata"]["name"] == name: continue
            links.append({
